...
usage() {
    cat <<'EOF'
Install CrossShare Server from GitHub releases.
...
Examples:
  ...
  curl -fsSL https://raw.githubusercontent.com/sunliang711/cshare/main/crossshare-server/install.sh | sh
  curl -fsSL https://raw.githubusercontent.com/sunliang711/cshare/main/crossshare-server/install.sh | sh -s -- -v v1.0.0
EOF
}
...
BINARY="crossshare-server-${VERSION}-${OS}-${ARCH}"
TAG="server/${VERSION}"
...
$SUDO install -m 755 "${TMPDIR}/${BINARY}" "${INSTALL_DIR}/crossshare-server"

echo "Installed crossshare-server ${VERSION} to ${INSTALL_DIR}/crossshare-server"
